...
export default function NewRequest() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await requestApi.createRequest(form);
  ...
  return (
    <Layout title="新建差旅申请" navItems={[{ path: '/employee/requests', label: '我的申请' }, { path: '/employee/requests/new', label: '新建申请' }]}>
      <form onSubmit={handleSubmit} style={{ maxWidth: '600px', backgroundColor: '#fff', padding: '24px', borderRadius: '8px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button type="submit" disabled={loading} style={{ padding: '8px 24px', border: 'none', borderRadius: '4px', backgroundColor: '#1677ff', color: '#fff', cursor: loading ? 'not-allowed' : 'pointer', fontSize: '14px', opacity: loading ? 0.7 : 1 }}>{loading ? '提交中...' : '提交申请'}</button>
